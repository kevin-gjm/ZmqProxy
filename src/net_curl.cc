...
bool NetCurl::s_bGlobalInitStatus = 0;
std::mutex NetCurl::m_MutexLock;


/*
	* 默认构造函数
*/
NetCurl::NetCurl():m_pCurl(NULL),
						   m_sIP(),
						   m_nPort(),
						   m_sUser(),
						   m_sPwd(),
...
/*
	...
	* 参数4：对方验证密码
*/
NetCurl::NetCurl(const std::string sIP,
...
//    catch(...)
//    {
//        EPRINT("~NetCurl api exception error(%d) \n", errno);
//    }
...
/*
	...
	* 返回值为int, 0表示成功，其他表示失败
*/
int NetCurl::releaseCurlResource()
{
	//判断参数的合法性
	if(NULL == m_pCurl)
	{
...
int NetCurl::setUserPwd()
{
	...
	std::string sUserPwd;
	if((!(m_sUser.empty())) && (!(m_sPwd.empty())))
	{
		sUserPwd = m_sUser + ":" + m_sPwd;
	   // std::string sUserPwd64 = base64_encode(sUserPwd);
		curl_easy_setopt(m_pCurl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
		curl_easy_setopt(m_pCurl, CURLOPT_USERPWD, (char*)sUserPwd.c_str());
	}
	return 0;
}
...
/*
	...
	* 参数1:最长时间秒数
*/
void NetCurl::setConnectTimeout(unsigned int nTime)
{
	m_contect_timeout = nTime;
}
...
/*
	...
	* 参数1:最长时间秒数
*/
void NetCurl::setTimeout(unsigned int nTime)
{
	m_timeout = nTime;
}
...
*/
//XML格式
#define  XML_FORMAT_STRING        "Content-Type: application/xml;charset=UTF-8;Except:"
//JSON格式
#define  JSON_FORMAT_STRING       "Content-Type: application/json;charset=UTF-8;Except:"
...
/*
	...
	* 返回值int,0表示成功,其它表示失败
*/
int NetCurl::setDataFormat(const int nFormat)
{
	if(NULL == m_pCurl)
	{
	...
	{
		//非空设置相应格式,空则不设置
		m_pHeaders = curl_slist_append(NULL, (char*)sFormatStr.c_str());
		if(NULL == m_pHeaders)
		{
...
/*
	...
	* 参数4：服务器密码
*/
void NetCurl::setServer(const std::string sIP,
							const unsigned int nPort,
							const std::string sUser,
							const std::string sPwd)
{
...
/*
	...
	* 参数1：URL路径
*/
void NetCurl::setUrlPath(const std::string sUrlPath)
{
	m_sUrlPath = sUrlPath;
}
...
/*
	...
	* 返回值, 数据所占字节数
*/
size_t NetCurl::httpDataWriter(void* buffer, size_t size, size_t nmemb, void* content)
{
	...
	{
		EPRINT("NetCurl httpDataWriter data error(%d) \n", errno);
		return 0;
	}
...
/*
	...
	* 返回值，数据所占字节数
*/
size_t NetCurl::httpReadFile(void* buffer, size_t size, size_t nmemb, void* file)
{
...
/*
	...
	* 返回值，数据所占字节数
*/
size_t NetCurl::httpWriteFile(void* buffer, size_t size, size_t nmemb, void* file)
{
...
*/
int NetCurl::sendMsg(const std::string& sMsg,
						 const int nMethod,
						 const int nFormat,
						 std::string& sRec)
{
...
/*
	...
	* 返回值int，0表示成功，其他表示失败
*/
int NetCurl::messagePublicMethod(const int nFormat,bool bneeduserpasswd)
{
	int nCode = -1;
	try
	{
		...
		std::string sUrl;
		sUrl = sUrl + "http://" + m_sIP + m_sUrlPath;
		DPRINT("sUrl: %s\n", sUrl.c_str());
		curl_easy_setopt(m_pCurl, CURLOPT_URL, (char*)sUrl.c_str());
		curl_easy_setopt(m_pCurl, CURLOPT_PORT, m_nPort);
		...
		//设置超时时间
		if(0 >=  m_contect_timeout)
		{
			 m_contect_timeout = CONNECT_DEFAULT_TIMEOUT;
		}
		curl_easy_setopt( m_pCurl, CURLOPT_CONNECTTIMEOUT,  m_contect_timeout);
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时, 其他表示失败
*/
int NetCurl::dealResCode(const CURLcode res)
{
	//输出返回码代表的意思
	int nCode = 0;
	...
	//http返回码
	long lResCode = 0;
	curl_easy_getinfo(m_pCurl, CURLINFO_RESPONSE_CODE, &lResCode);
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::putMsg(const std::string& sMsg, const int nFormat, std::string& sRec)
{
	...
	try
	{
		...
		{
			curl_easy_setopt(m_pCurl, CURLOPT_POSTFIELDS, (char*)sMsg.c_str());
		}
		...
		// 设置回调函数
		curl_easy_setopt(m_pCurl, CURLOPT_WRITEFUNCTION, httpDataWriter);
		curl_easy_setopt(m_pCurl, CURLOPT_WRITEDATA, (void*)&sRec);
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::deleteMsg(const std::string& sMsg, const int nFormat, std::string& sRec)
{
	CURLcode res = CURLE_OK;
	int nCode = -1;
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::postMsg(const std::string& sMsg, const int nFormat, std::string& sRec)
{
	CURLcode res = CURLE_OK;
	int nCode = -1;
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::getMsg(const std::string& sMsg, const int nFormat, std::string& sRec)
{
	CURLcode res = CURLE_OK;
	int nCode = -1;
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::downloadFile(const std::string& sFileName, const int nFormat)
{
	CURLcode res = CURLE_OK;
	FILE* pFile = NULL;
	int nCode = -1;
	...
	try
	{
		...
		// 设置回调函数
		curl_easy_setopt( m_pCurl, CURLOPT_WRITEFUNCTION, httpWriteFile);
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::uploadFileContent(const std::string& sFileName, const int nFormat, std::string& sRec)
{
	CURLcode res = CURLE_OK;
	FILE* pFile = NULL;
	...
	try
	{
		...
		// 设置回调函数
		curl_easy_setopt( m_pCurl, CURLOPT_READFUNCTION, httpReadFile);
		curl_easy_setopt( m_pCurl, CURLOPT_READDATA, pFile);
		curl_easy_setopt( m_pCurl, CURLOPT_POSTFIELDSIZE, (curl_off_t)fsize);
...
/*
	...
	* 返回文件名地址
*/
const char* NetCurl::getFileName(const char *path)
{
	if(!path)
	{
		return NULL;
...
/*
	...
	* 返回值int, 0表示成功, 1表示超时,其他表示失败
*/
int NetCurl::uploadFile(const std::string& sFileFullname, std::string& sRec)
{
	CURLcode res = CURLE_OK;
	int nCode = 0;
